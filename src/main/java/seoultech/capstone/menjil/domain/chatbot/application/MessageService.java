...
import seoultech.capstone.menjil.domain.chatbot.dto.request.AwsLambdaRequest;
import seoultech.capstone.menjil.domain.chatbot.dto.request.MessageRequest;
import seoultech.capstone.menjil.domain.chatbot.dto.response.AwsLambdaResponse;
...
public class MessageService {
    ...
    public boolean createWelcomeMessage(String roomId, String initiatorNickname, String recipientNickname) {
        ChatMessage welcomeMsg = new ChatMessage();
        String welcomeMessage = buildWelcomeMessageText(initiatorNickname, recipientNickname);
        Object messageList = null;
        MessageType messageType = MessageType.ENTER;
        LocalDateTime now = getCurrentTimeWithNanos();

        welcomeMsg.setWelcomeMessage(roomId, SenderType.AI, recipientNickname,
                welcomeMessage, messageList, messageType, now);

        return saveChatMessageInDb(welcomeMsg);
    }

    private String buildWelcomeMessageText(String init, String reci) {
        return "안녕하세요 " + init + "님!\n"
                + "저는 사용자 " + reci + "님의 AI 챗봇입니다. 당신의 질문을 입력해주세요";
    }
...
//                .forEach(awsLambdaResponses::add);
    ...
    private boolean saveChatMessageInDb(ChatMessage message) {
        try {
            ...
            log.error(">> messageRepository.save() error occurred ", e);
            return false;
        }
    }
